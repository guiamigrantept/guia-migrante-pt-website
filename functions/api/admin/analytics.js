...
export async function onRequestGet({request,env}){
  ...
  try{
    ...
    const totals=await env.CONTACT_DB.prepare("SELECT COUNT(*) AS total, SUM(CASE WHEN event_type='page_view' THEN 1 ELSE 0 END) AS page_views, SUM(CASE WHEN event_type='engaged_view' THEN 1 ELSE 0 END) AS engaged_views, SUM(CASE WHEN event_type='contact_submit' THEN 1 ELSE 0 END) AS contact_submits, SUM(CASE WHEN event_type='official_link_click' THEN 1 ELSE 0 END) AS official_clicks FROM analytics_events WHERE created_at >= datetime('now',?)").bind(modifier).first();
    const engagementMeta=await env.CONTACT_DB.prepare("SELECT MIN(created_at) AS engaged_since FROM analytics_events WHERE event_type='engaged_view'").first();
    ...
    const engagedPages=await env.CONTACT_DB.prepare("SELECT path, COUNT(*) AS n FROM analytics_events WHERE event_type='engaged_view' AND created_at >= datetime('now',?) GROUP BY path ORDER BY n DESC LIMIT 20").bind(modifier).all();
    ...
    const engagedLocales=await env.CONTACT_DB.prepare("SELECT locale, COUNT(*) AS n FROM analytics_events WHERE event_type='engaged_view' AND created_at >= datetime('now',?) GROUP BY locale ORDER BY n DESC").bind(modifier).all();
    ...
    return json({ok:true,days,totals:totals||{},engagement:{since:engagementMeta?.engaged_since||null},pages:pages.results||[],engagedPages:engagedPages.results||[],locales:locales.results||[],engagedLocales:engagedLocales.results||[],campaigns:campaigns.results||[],referrers:referrers.results||[],daily:daily.results||[]});
