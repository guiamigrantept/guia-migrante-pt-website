...
def clean_url(u):
 p=urlparse(u); host=p.netloc.lower(); path=re.sub(r'/+
...
,'',p.path) or '/'; q=p.query if any(x in p.query for x in ('contentId=','q=','canal=')) else ''
 # gov.pt migrated legacy www2 URLs to the current www.gov.pt structure.
 # Canonicalize before source IDs are generated so an obsolete hostname cannot
 # create false removals/quarantines again.
 if host=='www2.gov.pt':
  host='www.gov.pt'
  path=re.sub(r'^/(?:pt|pt-PT)(?=/)', '', path, flags=re.I) or '/'
  if path.casefold()=='/inicio/espaco-empresa/qualificacoes-profissionais':
   path='/guias/qualificacoes-profissionais'
 return urlunparse((p.scheme or 'https',host,path,'',q,''))
def sid(u): return 'src_'+hashlib.sha1(u.encode()).hexdigest()[:12]
def is_pdf(u): return urlparse(u).path.lower().endswith('.pdf')
def required(host,u): return host in HIGH and host not in INTERACTIVE and not is_pdf(u)
def watch_terms(s):
