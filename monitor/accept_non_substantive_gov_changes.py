...
def normalize_service_metadata(lines: list[str]) -> list[str]:
    ...
    ignored_labels = {
        ...
        'pedir niss',
        'nesta página', 'nesta pagina', 'on this page',
        'conteúdos relacionados', 'conteudos relacionados', 'related content',
    }
    toc_heading_patterns = (
        r'^quem pode(?:\s+.*)?$',
        r'^quando se pode(?:\s+.*)?$',
        r'^como e onde se pode(?:\s+.*)?$',
        r'^como e onde(?:\s+.*)?$',
    )
    while i < len(lines):
        low = lines[i].casefold()
        if low in ignored_labels or any(re.fullmatch(p, low) for p in toc_heading_patterns):
            i += 1
            continue
